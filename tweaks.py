...
def speed_up_tingle_jail_cutscene(self: WWRandomizer):
  ...
  This aggressively removes text boxes and wait times from the TC_RESCUE event
  while preserving the item get flow. Text boxes at indices 8, 16, 23, 30, 33
  and most WAIT actions are removed.
  """
  ...
  for action in camera.actions:
    timer_prop = action.get_prop("Timer")
    if timer_prop:
      timer_prop.value = max(1, timer_prop.value // 5)
  ...
  for action in camera.actions:
    if action.name in ["PAUSE", "UNITRANS", "FIXEDFRM"]:
      timer_prop = action.get_prop("Timer")
      if timer_prop:
        timer_prop.value = max(1, timer_prop.value // 10)
